...
that incorporates priors in modeling worker reliability. To apply these methods to our problem, we use negative sampling to simulate \pcm{is it really simulate or do you take the non-influencers answers from the crowd?} workers' answers of non-influencers \pcm{what do you take as negative samples? not sure I understand...}; we empirically determine the optimal sampling rates for each comparison method. 
...
To quantitatively show the effect of modeling worker reliability as a latent variable, we depict in Figure~\ref{fig:worker_reliability} two examples of workers of similar reliability yet providing different number of answers. We observe that their reliability, as estimated by \sys-EM, are hardly distinguishable; however, \sys provides extra insight on the estimated reliability of worker from the left being more trustable than the one from the right \pcm{not sure I follow; what is left / right and where is sys-EM?}.
...
\begin{axis}[
    ...
    ytick={0.6,0.65,0.7,0.75,0.8},
       ...
       anchor=north,legend columns= 4, font = \LARGE},
    yticklabel style = {font=\huge,xshift=0ex},
    xticklabel style = {font=\huge,yshift=0ex},
    ylabel style ={font = \huge, yshift=2ex},
